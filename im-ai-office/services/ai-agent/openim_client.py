...
def _configure():
    global API_ADDRESS, ADMIN_TOKEN
    import os
...
def send_group_notice(group_id, text, send_id="openIMAdmin"):
    """群里发通知/确认卡。"""
    return send_msg(send_id, group_id, text)


def send_private_confirm(group_id, user_id, text, send_id="openIMAdmin"):
    """私聊发送者做消歧确认（低打扰）。"""
    return send_msg(send_id, "", text, recv_id=user_id)


def send_confirm_card(recv_id, card_json, group_id=None, send_id="openIMAdmin"):
