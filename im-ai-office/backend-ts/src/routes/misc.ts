...
miscRoutes.get("/api/events/stream", (c) => {
...
miscRoutes.get("/api/ai_dm", async (c) => {
...
miscRoutes.post("/api/ai_dm/read", async (c) => {
  const body = await c.req.json().catch(() => ({}));
  await aiDmMarkRead(body?.sender_id ?? null);
  return c.json({ ok: true });
});

miscRoutes.get("/api/audit", async (c) => {
